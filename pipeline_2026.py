"""
pipeline_2026.py - ETL fuer Haushaltsplan Suhl 2026
Identisch zu pipeline_2024.py, aber mit 2026-Konfiguration (COL_DEFS, Ground Truth).

Spalten-Koordinaten wurden per pdfplumber-Probe auf dem neuen PDF
(knowledge/HH-Plan Stadt Suhl 2026/HHPlan 2026.pdf) verifiziert:
Sowohl die TP-Chunks (x-Grid 265/340/392/437/482/527/580) als auch der
Gesamtproduktplan-Finanzplan-Chunk (x-Grid 225/293/348/401/450/500/555)
sind PIXEL-IDENTISCH zu den in pipeline_2024.py verwendeten Koordinaten -
nur die Jahreszahlen der Spalten verschieben sich um +2.
"""
...
CHUNKS_DIR = os.path.join(BASE_DIR, "pdf_chunks_2026")
DB_PATH    = os.path.join(BASE_DIR, "suhl_haushalt_2025.db")  # selbe DB, anderes haushaltsplan_jahr
FAIL_LOG   = os.path.join(BASE_DIR, "parsing_failures_2026.log")
HAUSHALTSPLAN_JAHR = 2026
BATCH_SIZE = 500

# Spalten-Definitionen: (x_min, x_max, daten_jahr, wert_typ)
# Verifiziert per Probe auf pdf_chunks_2026/tp_01_Verwaltungsfuehrung.pdf (Seiten 3,6,9):
# Jahres-Labels im PDF bei x=306,353,399,444,489,534 (identisch zu 2024-PDF)
COL_DEFS = [
    (265, 340, 2024, "IST_ERGEBNIS"),
    (340, 392, 2025, "ANSATZ_VORJAHR"),
    (392, 437, 2026, "PLAN_ANSATZ"),
    (437, 482, 2027, "FINANZPLANUNG"),
    (482, 527, 2028, "FINANZPLANUNG"),
    (527, 580, 2029, "FINANZPLANUNG"),
]

# Account-Code-Format: PPPPPP.KKKKKK(K) bei x=55-125
KONTO_LINE_RE = re.compile(r"^(\d{5,6})\.(\d{6,7})$")

# Spalten-Definitionen für Gesamtproduktplan Finanzplan (3.5 "Finanzplan mit Konten")
# Verifiziert per Probe auf pdf_chunks_2026/00_3_Gesamtproduktplan.pdf (Seiten 28, 43):
# Jahresspalten liegen ~37px weiter links als in TP-Chunks (identisch zu 2024-PDF)
COL_DEFS_GESAMT = [
    (225, 293, 2024, "IST_ERGEBNIS"),
    (293, 348, 2025, "ANSATZ_VORJAHR"),
    (348, 401, 2026, "PLAN_ANSATZ"),
    (401, 450, 2027, "FINANZPLANUNG"),
    (450, 500, 2028, "FINANZPLANUNG"),
    (500, 555, 2029, "FINANZPLANUNG"),
]
...
# Abschnitt "3.5 Finanzplan mit Konten" laut TOC: Dok-Seiten 136-153 (Chunk beginnt bei
# Dok-Seite 110 -> relative Indizes 26-43). Seite 26 (Index) ist die reine Titelseite
# des Abschnitts (keine Kontendaten, harmlos); Seite 43 (Index) ist die letzte Seite vor
# "4. Investitionsplan" (Dok-Seite 154, ausserhalb dieses Chunks). Verifiziert per Probe.
GESAMT_FP_START_PAGE = 26   # 0-basiert (= Seite 27 im Chunk, dok-Seite 136, Abschnittstitel)
GESAMT_FP_END_PAGE   = 43   # 0-basiert, inklusiv (= Seite 44 im Chunk, dok-Seite 153)
...
# Ground Truth aus der Haushaltssatzung 2026 §1 (Seiten 21-22 des PDFs,
# "ordentliche" Ertraege/Aufwendungen/Einzahlungen/Auszahlungen)
GROUND_TRUTH = {
    (4, "PLAN_ANSATZ"): 141_952_900.00,
    (5, "PLAN_ANSATZ"): 144_971_430.00,
    (6, "PLAN_ANSATZ"): 135_116_810.00,
    (7, "PLAN_ANSATZ"): 134_286_770.00,
}
...
def extract_page(page, fail_log, page_label: str) -> list:
    ...
    for y, row in line_list:
        # Account-Code: PPPPPP.KKKKKK bei x=50–125 (verifiziert: x~=70)
        konto_match = None
        konto_word = None
        for w in row:
        ...
        if konto_match:
            ...
            if same_line_values:
                ...
                # Liste statt Tupel, da konto_bez bei mehrzeiligem Umbruch
                # unten noch ergänzt werden kann (siehe extra_bez).
...
def extract_page_gesamt(page, fail_log, page_label: str) -> list:
    """
    Parst eine Seite des Gesamtproduktplan-Finanzplans (Abschnitt 3.5).
...
def process_gesamtproduktplan_finanzplan(con, db: DbContext, fail_log) -> dict:
    ...
    Gesamtproduktplan Finanzplan mit Einzelkontengliederung (Abschnitt 3.5).
...
def run_validation(con) -> int:
    errors = 0
    print("\n--- Ground Truth Validierung (Haushaltssatzung 2026) ---")
    labels = {
        4: "Ertraege    KK4",
        5: "Aufwendungen KK5",
        6: "Einzahlungen KK6",
    ...
    # Die "ordentlichen" Eckwerte der Satzung (§1) schliessen laut Formelsystem
    # des Teilergebnis-/Teilfinanzplans (Zeilen "Saldo der ordentlichen Ein-/
    # Auszahlungen", VOR Verrechnung interner Leistungsbeziehungen und VOR
    # Investitionstaetigkeit) folgende Unterkontenklassen aus:
    #   KK4 -> 48xxxxx  Ertraege aus internen Leistungsbeziehungen
    #   KK5 -> 58xxxxx  Aufwendungen aus internen Leistungsbeziehungen
    #   KK6 -> 68xxxxx  Einzahlungen aus Investitionstaetigkeit
    #          69xxxxx  Einzahlungen aus internen Leistungsbeziehungen
    #   KK7 -> 78xxxxx  Auszahlungen aus Investitionstaetigkeit
    #          79xxxxx  Auszahlungen aus internen Leistungsbeziehungen
    # Diese Konten werden auf TP-Ebene sehr wohl korrekt je Produkt erfasst
    # (z.B. fuer den Investitionsplan) - sie zaehlen nur NICHT zur "ordentlichen"
    # Satzungs-Summe. Verifiziert per Diagnose: ohne diesen Ausschluss weicht
    # KK6/7 um exakt die hier ausgeschlossenen Summen ab (68/69/78/79-Betraege
    # aus den TP-Chunks kommen zusaetzlich zur separat bereits vollstaendigen
    # GESAMT-FP-Zweitpass-Summe hinzu -> Doppelzaehlung ggue. der Satzung).
    EXCLUDE_PREFIXES = {
        4: ("48",),
        5: ("58",),
        6: ("68", "69"),
    ...
    for (kk_nr, wert_typ), soll in GROUND_TRUTH.items():
        exclude = EXCLUDE_PREFIXES.get(kk_nr, ())
        exclude_clause = ""
        params = [HAUSHALTSPLAN_JAHR, wert_typ, kk_nr]
        if exclude:
            placeholders = ", ".join("?" for _ in exclude)
            exclude_clause = f" AND substr(k.konto_nummer, 1, 2) NOT IN ({placeholders})"
...
def main():
    ...
    if not os.path.exists(CHUNKS_DIR):
        print(f"[FEHLER] Chunk-Verzeichnis nicht gefunden: {CHUNKS_DIR}. Bitte smart_split_2026.py ausfuehren.")
        sys.exit(1)

    chunk_files = sorted(glob(os.path.join(CHUNKS_DIR, "tp_*.pdf")))
    if not chunk_files:
        print(f"[FEHLER] Keine tp_*.pdf in {CHUNKS_DIR}. Bitte smart_split_2026.py ausfuehren.")
        sys.exit(1)
    ...
    # Zweiter ETL-Pass: laufende KK6/7 aus Gesamtproduktplan Finanzplan
    print(f"[GESAMT-FP] {os.path.basename(GESAMT_FINANZPLAN_CHUNK)}  (Seiten {GESAMT_FP_START_PAGE+1}-{GESAMT_FP_END_PAGE+1})")
    try:
