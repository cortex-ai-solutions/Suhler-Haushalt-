"""
pipeline_2023.py - ETL fuer Haushaltsplan Suhl 2023
Identisch zu pipeline_2024.py, aber mit 2023-Konfiguration (COL_DEFS, Ground Truth).
"""
...
CHUNKS_DIR = os.path.join(BASE_DIR, "pdf_chunks_2023")
DB_PATH    = os.path.join(BASE_DIR, "suhl_haushalt_2025.db")
FAIL_LOG   = os.path.join(BASE_DIR, "parsing_failures_2023.log")
HAUSHALTSPLAN_JAHR = 2023
BATCH_SIZE = 500

# Spalten-Definitionen: (x_min, x_max, daten_jahr, wert_typ)
# Jahres-Labels im PDF bei x=293,341,386,432,477,523 (Teilergebnisplan/Teilfinanzplan)
COL_DEFS = [
    (265, 335, 2021, "IST_ERGEBNIS"),
    (335, 385, 2022, "ANSATZ_VORJAHR"),
    (385, 430, 2023, "PLAN_ANSATZ"),
    (430, 478, 2024, "FINANZPLANUNG"),
    (478, 523, 2025, "FINANZPLANUNG"),
    (523, 580, 2026, "FINANZPLANUNG"),
]

# Account-Code-Format: PPPPPP.KKKKKK(K) bei x=50-125
KONTO_LINE_RE = re.compile(r"^(\d{5,6})\.(\d{6,7})$")

# Spalten-Definitionen fuer Gesamtproduktplan Finanzplan
# Jahres-Labels im PDF bei x=252,309,366,419,469,518 (Gesamtfinanzplan)
COL_DEFS_GESAMT = [
    (225, 290, 2021, "IST_ERGEBNIS"),
    (290, 348, 2022, "ANSATZ_VORJAHR"),
    (348, 402, 2023, "PLAN_ANSATZ"),
    (402, 452, 2024, "FINANZPLANUNG"),
    (452, 502, 2025, "FINANZPLANUNG"),
    (502, 560, 2026, "FINANZPLANUNG"),
]

# Account-Code-Format im Gesamtproduktplan Finanzplan: 7-stellig, kein Punkt, bei x=65-95
PLAIN_KONTO_RE = re.compile(r"^(\d{7})$")

# Chunk: nur Gesamtfinanzplan-Seiten (PDF 125-141, alle 17 Seiten enthalten KK6/7-Daten)
GESAMT_FINANZPLAN_CHUNK = os.path.join(CHUNKS_DIR, "00_Gesamtfinanzplan.pdf")
GESAMT_FP_START_PAGE = 0    # 0-basiert: erste Seite im Chunk
GESAMT_FP_END_PAGE   = 16   # 0-basiert, inklusiv: letzte Seite im Chunk (17 Seiten)
...
# Ground Truth aus der Haushaltssatzung 2023 §1
GROUND_TRUTH = {
    (4, "PLAN_ANSATZ"): 129_315_250.00,
    (5, "PLAN_ANSATZ"): 129_724_490.00,
    (6, "PLAN_ANSATZ"): 124_088_080.00,
    (7, "PLAN_ANSATZ"): 120_677_820.00,
}
...
def parse_german_number(s: str):
    """Konvertiert deutschen Haushaltsbetrag in float; None bei ungueltigem Format."""
...
def extract_page(page, fail_log, page_label: str) -> list:
    ...
    for y, row in line_list:
        ...
        if konto_match:
            # KK6/7 (Einzahlungen/Auszahlungen) werden vollständig im Gesamtfinanzplan
            # erfasst. TP-Chunks enthalten dieselben Konten im Teilfinanzplan → überspringen
            # um Doppelzählung zu vermeiden.
            if konto_match.group(2)[0] in ("6", "7"):
                pending = None
                continue
...
def extract_page_gesamt(page, fail_log, page_label: str) -> list:
    """
    Parst Seiten des Gesamtproduktplan Finanzplans.
    Account-Code: 7-stellige Zahl ohne Punkt bei x=65-95.
    Nur KK6/7 (laufende Einzahlungen/Auszahlungen).
    """
    try:
        words = page.extract_words(
            x_tolerance=3, y_tolerance=3,
...
# ---------------------------------------------------------------------------
# Zweiter ETL-Pass: laufende KK6/7 aus Gesamtfinanzplan
# ---------------------------------------------------------------------------
...
def run_validation(con) -> int:
    errors = 0
    print("\n--- Ground Truth Validierung (Haushaltssatzung 2023 §1) ---")
    labels = {
        4: "Ertraege    KK4",
        5: "Aufwendungen KK5",
        6: "Einzahlungen KK6",
    ...
    for (kk_nr, wert_typ), soll in GROUND_TRUTH.items():
        ...
        diff = ist - soll
        ok   = abs(diff / soll) < 0.005  # <0,5% Toleranz
...
# ---------------------------------------------------------------------------
# Teilplan-Bezeichnungen in DB aktualisieren
# ---------------------------------------------------------------------------
...
def main():
    ...
    if not os.path.exists(CHUNKS_DIR):
        print(f"[FEHLER] Chunk-Verzeichnis nicht gefunden: {CHUNKS_DIR}. Bitte smart_split_2023.py ausfuehren.")
        sys.exit(1)

    chunk_files = sorted(glob(os.path.join(CHUNKS_DIR, "tp_*.pdf")))
    if not chunk_files:
        print(f"[FEHLER] Keine tp_*.pdf in {CHUNKS_DIR}. Bitte smart_split_2023.py ausfuehren.")
        sys.exit(1)
    ...
    print(f"[GESAMT-FP] {os.path.basename(GESAMT_FINANZPLAN_CHUNK)}  (Seiten 1-17 im Chunk)")
    try:
    ...
    if gt_errors > 0:
        print(f"\n  !! {gt_errors} Ground-Truth-Abweichung(en) -- COL_DEFS pruefen!")
        sys.exit(2)
    else:
        print("\n  Alle Ground-Truth-Checks bestanden.")
